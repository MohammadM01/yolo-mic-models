...
function ModelsTesting() {
  ...
  const startInterview = async () => {
    try {
      ...
      if (!cameraInitialized) {
        console.warn('Camera initialization failed, continuing with audio only');
      }
      ...
      if (data.success) {
        ...
        console.log('=== FIRST CLICK: Interview started successfully - Cycle 1 (Video + Audio) ===');
        console.log('Data collection will run continuously in 10-second cycles');
        console.log('Summaries logged to console every 10 seconds');
        console.log('Click the button again to stop processing and view all results');
  ...
  const completeCycle = async () => {
    try {
      console.log(`Completing Cycle ${cycleNumber} - 10 seconds of data collected`);
      ...
      const cycleData = {
        ...
        summary: {
          ...
          eyeContactPercentage: (realTimeData.filter(d => d.results[1]).length / realTimeData.length * 100).toFixed(2),
          commonExpression: getMostCommonExpression(realTimeData),
          commonPosture: getMostCommonPosture(realTimeData),
          suspiciousPercentage: (realTimeData.filter(d => d.results[3]).length / realTimeData.length * 100).toFixed(2)
        },
        timestamp: Date.now()
      };
  ...
  const stopInterview = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log('=== SECOND CLICK: Data processing stopped successfully ===');
        ...
        const summaryResponse = await fetch(`${API_BASE}/api/interview/summary`);
  ...
  const renderInterviewData = () => {
    ...
    return (
      <div className="text-white p-4 space-y-4">
        <h2 className="text-2xl font-bold text-center mb-4">Interview Analysis Results</h2>
        ...
        <div className="bg-white/10 rounded-lg p-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div>Suspicious Activity: {interviewData.suspiciousPercentage}%</div>
          </div>
        </div>

        <div className="bg-white/10 rounded-lg p-3">
          <h3 className="text-lg font-semibold mb-2">Per Cycle Summaries ({allCyclesData.length})</h3>
          <div className="max-h-96 overflow-y-auto space-y-4">
            {allCyclesData.map((cycle, index) => (
              <div key={index} className="p-3 bg-white/5 rounded">
                <h4 className="font-semibold mb-2">Cycle {cycle.cycleNumber} (10s)</h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
    ...
    <div className="min-h-screen flex flex-col">
      <main className="bg-[#1A233B] flex-1 justify-center relative p-5">
        ...
        <div className="bg-[#9370DB] w-[47%] h-[95%] max-h-[100%] rounded-lg absolute top-5 right-5 overflow-y-auto">
          {isRecording ? (
            <div className="text-white p-4 text-center">
              <h2 className="text-2xl font-bold mb-4">Data Collection in Progress</h2>
              <p className="text-lg mb-4">Collecting data in 10-second cycles...</p>
              <div className="space-y-2 text-sm">
                <p>Cycles Completed: {allCyclesData.length}</p>
                <p>Current Cycle: {cycleNumber}</p>
                <p>Progress: {sessionDuration}/10 seconds</p>
                <p className="text-yellow-300">Check console for cycle summaries</p>
              </div>
            </div>
          ) : renderInterviewData()}
        </div>
        ...
        <div className="absolute bottom-5 left-5 flex gap-4">
          ...
          >
            {isLoading ? 'Loading...' : isFirstClick ? 'Press to Start' : 'Press to Stop & View Data'}
          </button>
